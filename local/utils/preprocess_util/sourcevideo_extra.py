            ...
            if d_data['data']:
                for face in d_data['data']:
                    ...
                    if face['confidence'] < 0.8:
                        ...
                        crop = img[int(left-50):int(right+50),int(top-50):int(bottom+50),:]
        ...
        if split_frames is not None:
            ...
            for j, img in enumerate(split_frames):
                ...
                if not flag:
                    ...
                    extra_flag = True
            #只要有不属于这个up主的frame，这段10秒视频就丢弃,不然就保存
            if extra_flag:
                ...
                ffmpeg_command = "ffmpeg -hwaccel cuda -c:v h264_cuvid -ss {} -i {} -c:v h264_nvenc -t {} -y -r 25 {} >> exp/log/ffmpeg.log 2>&1"
